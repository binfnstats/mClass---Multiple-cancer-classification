...
def read_sample_files(path, gene_index_dict):
    ...
    for file in range(1, len(file_name_list)):
        ...
        all_sample_cancer_snp_data = \
            np.vstack((all_sample_cancer_snp_data, sample_snp_data))

    return (all_sample_cancer_snp_data, file_name_list)


def write_cancer_snp_data_into_file(
    data,
    all_cancer_gene,
    sample_name_list,
    cancer_name,
...
def main():
    ...
    os.chdir(data_path)
    cancer_directoreis = [name for name in os.listdir('.')
    ...
    for cancer in range(len(cancer_directoreis)):
        cancer_directory_path = os.path.join(data_path,
                cancer_directoreis[cancer])
    ...
    for cancer in range(0, len(cancer_directoreis)):
        ...
        cancer_directory_path = os.path.join(data_path,
                cancer_directoreis[cancer])
        (individual_cancer_snp_data, sample_name_list) = \
            read_sample_files(cancer_directory_path, gene_index_dict)

        # print(cancer_name, len(sample_name_list), np.sum(np.sum(individual_cancer_snp_data, axis = 1)))

        os.chdir(data_path)
        write_cancer_snp_data_into_file(np.array(individual_cancer_snp_data),
                all_cancer_gene, sample_name_list, cancer_name)
